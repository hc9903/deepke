...
# 自定义模型存储的路径
cw = os.path.abspath('.')
fp = cw + os.path.sep + 'cnn_epoch22.pth'
...
def main(cfg):
    ...
    insert = input("是否使用默认预测文件y/n[如果你上面跳过了生成预测集直接Enter]:")

    name = ''
    if insert.strip() != '' and insert.strip() in {'n', 'N'}:
      name = "data" + os.path.sep


    file = open(cw + os.path.sep + name + 'out.txt', encoding = "utf8")
    out = open(cw + os.path.sep + 'predict.txt', 'w', encoding = "utf8")
    #last_line = ''
    for line in file:
        # get predict instance
